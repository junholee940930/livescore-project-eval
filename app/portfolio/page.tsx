  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "16px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}>
        <Link href="/" style={{ fontSize: "14px", color: "#6B7280", textDecoration: "none" }}>← 시장현황</Link>
        <span style={{ color: "#D1D5DB" }}>|</span>
        <h1 style={{ fontSize: "16px", fontWeight: 700, color: "#111827", margin: 0 }}>내 포트폴리오</h1>
        <span style={{ fontSize: "14px", color: "#6B7280", background: "#F3F4F6", padding: "2px 8px", borderRadius: "10px" }}>
      ...
      {allEntries.length === 0 ? (
        <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: "6px", padding: "48px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: "14px", color: "#9CA3AF" }}>프로젝트 상세 페이지에서 주식을 구매하면 여기에 표시됩니다.</div>
          <Link href="/" style={{ display: "inline-block", marginTop: "16px", padding: "8px 20px", background: "#111827", color: "#fff", borderRadius: "4px", textDecoration: "none", fontSize: "14px", fontWeight: 600 }}>
            시장으로 이동 →
          </Link>
        </div>
      ) : (
                  ...
                  return (
                    <tr key={`${b.buyerName}-${e.projectId}-${i}`} style={{ borderBottom: "1px solid #F1F5F9" }}>
                      ...
                      <td style={{ padding: "9px 12px" }}>
                        ...
                        {e.excelId && <div style={{ fontSize: "14px", color: "#4B9FEA" }}>{e.excelId}</div>}
                      </td>
                      ...
                      <td style={{ padding: "9px 12px", color: "#6B7280", fontSize: "14px" }}>{e.purchasedAt.slice(0, 10)}</td>
                      <td style={{ padding: "9px 12px", textAlign: "center" }}>
                        <Link href={`/projects/${e.projectId}`} style={{ fontSize: "14px", color: "#3A7BD5", textDecoration: "none", fontWeight: 600 }}>보기</Link>
                      </td>
                    </tr>
                  );
                })
